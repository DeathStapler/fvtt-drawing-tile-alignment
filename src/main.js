...
class TDAlignTool {
	...
	static horizontalSpacing() {
		...
		drawings.sort((a, b) => (a.center.x + TDAlignTool.getWidth(a)) - (b.center.x + TDAlignTool.getWidth(b)));
		...
		for (const idx in drawings) {
			...
			updates[drawing.constructor.name].push({
				_id: drawing.document.id,
				x: newCenter - TDAlignTool.getWidth(drawing) / 2
			});
		}
	...
	static verticalSpacing() {
		...
		if (drawings.length > 2) {
			...
			for (const idx in drawings) {
				...
				updates[drawing.constructor.name].push({
					_id: drawing.document.id,
					y: newCenter - TDAlignTool.getHeight(drawing) / 2
				});
			}
			TDAlignTool.doUpdates(updates);
		}
		...
		if (drawings.length > 1) {
			...
			drawings.forEach(drawing => {
				if (commonCenter != drawing.center.y) {
					...
					updates[drawing.constructor.name].push({
						_id: drawing.document.id,
						y: commonCenter - TDAlignTool.getHeight(drawing) / 2
					});
				}
			});
		...
		if (drawings.length > 1) {
			...
			drawings.forEach(drawing => {
				if (commonCenter != drawing.center.x) {
					...
					updates[drawing.constructor.name].push({
						_id: drawing.document.id,
						x: commonCenter - TDAlignTool.getWidth(drawing) / 2
					});
	...
	static horizAlignLeft() {
		...
		drawings.forEach(drawing => {
			leftPoints.push(drawing.document.x);
		});
	...
	static horizAlignRight() {
		...
		drawings.forEach(drawing => {
			rightPoints.push(drawing.document.x + TDAlignTool.getWidth(drawing));
		});
		...
		drawings.forEach(drawing => {
			...
			updates[drawing.constructor.name].push({
				_id: drawing.document.id,
				x: commonRight - TDAlignTool.getWidth(drawing)
			});
		});
		TDAlignTool.doUpdates(updates);
	}
	...
	static vertAlignBottom() {
		...
		drawings.forEach(drawing => {
			bottomPoints.push(drawing.document.y + TDAlignTool.getHeight(drawing));
		});
		...
		drawings.forEach(drawing => {
			...
			updates[drawing.constructor.name].push({
				_id: drawing.document.id,
				y: commonBottom - TDAlignTool.getHeight(drawing)
			});
		});
		TDAlignTool.doUpdates(updates);
	}
	...
	static vertAlignTop() {
		...
		drawings.forEach(drawing => {
			topPoints.push(drawing.document.y);
		});
	...
	static findTopLeftMostPos(objects) {
		...
		const topLeft = objects.reduce((best, current) => {
			if (current.document.x < best.document.x) return current;
			if (current.document.x === best.document.x && current.document.y < best.document.y) return current;
			return best;
		}, objects[0]);

		return {
			x: topLeft.document.x,
			y: topLeft.document.y
		};
	}
	...
	static positionInGrid(objects) {
		...
		objects.forEach((obj, index) => {
			...
			updates[obj.constructor.name].push({
				_id: obj.document.id,
				x: topLeftPos.x + x,
				y: topLeftPos.y + y
			});
	...
	static positionInCircle(objects) {
		...
		objects.forEach((obj, index) => {
			...
			updates[obj.constructor.name].push({
				_id: obj.document.id,
				x: x - TDAlignTool.getWidth(obj) / 2,
				y: y - TDAlignTool.getHeight(obj) / 2
			});
		});
		TDAlignTool.doUpdates(updates);
	}
	...
	static getHeight(placeable) {
		return placeable?.bounds?.height ?? 0;
	}

	static getWidth(placeable) {
		return placeable?.bounds?.width ?? 0;
	}
